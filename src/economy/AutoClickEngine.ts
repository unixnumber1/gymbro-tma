...
export function calcTotalCoinsPerSecond(
  ...
  const shopBoostMult  = shopPassiveBoostActive ? 2 : 1
  const permBonus      = 1 + permanentPassiveBonus
  ...
  return baseTotal * passiveMult * appearanceMult * prestigeMult * goalsMult * shopBoostMult * permBonus
}
